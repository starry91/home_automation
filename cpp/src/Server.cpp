...
int UDPListenAndRespond(int port_number) {
	...
	while (1) {
		...
		if ((len = recvfrom(sock_descriptor, udp_buf, sizeof(udp_buf), 0,
		...
		std::cout << "client address: "
				<< std::string(inet_ntoa(cli_addr.sin_addr)) << std::endl;
		std::cout << "client len: " << clilen << std::endl;
		char p[] = "HI";
		if (trim(temp) == "DISCOVER_SERVER_REQUEST") {
			if (sendto(sock_descriptor, p, strlen(p), 0,
