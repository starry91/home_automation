...
int main(int argc, char *argv[]) {
	...
	int py_sd = connect_to_py_server();
	if(read_from_config(data, json_data, py_sd) < 0)
	...
	while (1)
	{
		...
		for (int i = 0; i < client_data.getClientListSize(); i++)
		{
			...
			{
				int status = check_client_activity_update_and_respond(readfds, client_data, data, json_data, sd, py_sd);
				if(status > 0)
					cout << "error" <<endl;
			}
			else
